...
- `wear/` — Wear OS version (Watch Face Format, code-free)
...
**The AVD must be Wear OS 6 (API 36) or newer.** The face declares Watch
Face Format 4, and an older runtime refuses to load it. The long-standing
`chronology_wear` AVD is API 34 and can no longer render this face; it is
still useful for checking the format-1 history in git. Create a current one
at 1080x1080 for store-quality screenshots:
```
sdkmanager "system-images;android-36;android-wear;arm64-v8a"
avdmanager create avd -n arcminute_wear -k "system-images;android-36;android-wear;arm64-v8a" -d wearos_large_round
~/Library/Android/sdk/emulator/emulator -avd arcminute_wear -no-window -no-audio &
```
To open the style editor: long-press the watchface, tap the pencil.

## Watch Face Format details

Declarative WFF v4 (`minSdk 36`, Wear OS 6+), for the ambient transition
attributes on `Variant`. The format version in
`wear/wff/src/main/AndroidManifest.xml` and `minSdk` in
`wear/wff/build.gradle.kts` must move together: Play filters by `minSdk`, so
a mismatch offers the face to watches that cannot render it. Version 5 needs
Wear OS 7 and adds nothing this face uses — its features are blend modes,
multi-line text metrics, `Stroke` joins and nested `ListOption` settings.
Code-free APK: the whole watchface is
...
Ambient is always dark and is **not** a user setting. The `darkAmbient`
toggle was removed in 1.2.0: the face disc covers the screen, so leaving it
lit put every color but black over Play's [15% illuminated-pixel
guidance](https://developer.android.com/docs/quality-guidelines/wear-app-quality#wff).

`Variant` carries only numeric values — it can swap `alpha`, never a color —
so ambient is an alpha crossfade, not a recolor. A second dial in fixed
light colors is unavoidable; marks drawn in a light face's dark text would
vanish against the black.

Two things make it cheap:

- The `Scene` `backgroundColor` is `#000000`, not the face color, because
  it is not transformable. The face color is painted by a full-screen
  `Rectangle` instead, which *can* fade out. Left on the `Scene` it would
  keep lighting the crescent the dial disc does not reach, all night.
- Every interactive layer — ground, dial, complication — carries
  `<Variant mode="AMBIENT" target="alpha" value="0" />` on its group, so the
  runtime drops the whole subtree instead of drawing it under a veil. One
...
for a mode change, and **the runtime discards both if they sum past 1.0**.
